...
/**
 * 【支払】各画面の「やり取り（コメント）」。請求側（{@see BillingMessageController}）
 * と同じ仕組みで、建物予算項目（t_building_budget_items）単位の1スレッドに集約する。
 *
 * ルートは支払取引先（t_payable_partners）でバインドするが、コメントはポリモーフィックで項目に
 * ぶら下がるため、**同一項目なら支払・請求のどちらの画面から開いても同じスレッド**が見える。
 * チャット UI（モーダル）から fetch で利用するため JSON を返す。
 */
class PayableMessageController extends Controller
{
     ...
     */
    public function index(TPayablePartner $partner): JsonResponse
    {
    ...
    public function store(StoreCommentRequest $request, TPayablePartner $partner): JsonResponse
    {
