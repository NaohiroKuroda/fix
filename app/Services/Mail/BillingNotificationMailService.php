 ...
 */
class BillingNotificationMailService
{
    ...
    private function send(array $partnerIds, string $title, string $exp, string $notice, callable $linkBuilder): int
    {
        ...
        $partners = $this->repository->findPartnersForMail($ids);
        if ($partners->isEmpty()) {
            Log::warning('【請求】通知メール：送信対象の請求取引先がありません', [
                'title' => $title,
                'partnerIds' => $ids,
            ]);
        ...
        /** @var array<int, list<array{legacy: EstimateUnitCompany, partner: TBillingPartner}>> $grouped */
        $grouped = [];
        foreach ($partners as $partner) {
            $legacy = $legacyCompanies->get((int) $partner->source_id);
            if ($legacy === null) {
                Log::warning('【請求】通知メール：移行元の見積業者が見つからないため対象外にします', [
                    'title' => $title,
        ...
        foreach ($grouped as $companyId => $rows) {
            ...
            if ($toAddresses === [] && (array) config('mail_queue.override_to', []) === []) {
                Log::warning('【請求】通知メール：宛先が無いため送信しません', [
                    'title' => $title,
                    'companyId' => $companyId,
                ]);
            ...
            try {
                ...
                Log::info('【請求】通知メールをメールキューへ登録しました', [
                    'title' => $title,
                    'companyId' => $companyId,
                    'partnerIds' => $ids,
                    'to' => $toAddresses,
