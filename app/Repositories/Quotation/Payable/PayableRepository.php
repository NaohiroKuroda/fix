 ...
 *
 * 旧 felix_total スキーマ版（{@see QuotationRepository}）と同じ {@see PayableRepositoryInterface}
...
class PayableRepository implements PayableRepositoryInterface
{
     ...
     *
     * @param  list<int>  $partnerIds  見積先（t_payable_partners.id）の配列
     ...
     *
     * @param  list<int>  $partnerIds  見積先（t_payable_partners.id）
    ...
    public function rejectManagerApproval(int $partnerId, string $reason): int
    {
        ...
        $targets = $this->filterByStatus($this->mapSourceIds([$partnerId]), 'APPLIED');

        return DB::transaction(function () use ($partnerId, $targets): int {
     ...
     */
    private function mapSourceIds(array $partnerIds): array
    {
        if ($partnerIds === []) {
            return [];
        }
     ...
     *
     * @param  int  $partnerId  t_payable_partners.id
     */
