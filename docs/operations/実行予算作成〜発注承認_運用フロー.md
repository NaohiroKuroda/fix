...
| 新テーブル | 内容 |
| --- | --- |
| `t_payable_quotations` | 回答1件＝1レコード。`amount_excluding_tax` / `quotation_date` / `file_url` など。最新1件だけ `is_latest = true`（旧ロジック「`type=1` を date DESC, id DESC の先頭」を鏡写し） |
...
| 見積管理の状態遷移 | `fix/app/Repositories/BuildingQuotationRepository.php` |
| felix_total 連携（HTTP） | `fix/app/Services/FelixTotal/FelixTotalQuoteRequestGateway.php` |
| 発注〜請求の状態遷移 | `fix/app/Repositories/OrderDeliveryRepository.php` |
