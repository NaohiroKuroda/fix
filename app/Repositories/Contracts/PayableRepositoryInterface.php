...
interface PayableRepositoryInterface
{
     ...
     */
    public function forPayableManagement(array $filters, int $perPage, string $mode): LengthAwarePaginator;

    /**
     * 見積依頼送信（相見積依頼の記録）。指定の支払取引先（t_payable_partners.id）ごとに
     * `t_payable_quotation_requests` を1件作成する。既に依頼履歴がある見積先は重複して作成しない。
     *
     * @param  list<int>  $partnerIds  支払取引先（t_payable_partners）の ID 配列
     * @return int 実際に依頼を記録した件数
     */
    public function recordQuoteRequests(array $partnerIds): int;

    /**
     * 業者選定の確定。指定の支払取引先（t_payable_partners.id）を採用（adoption_flg=1）し、
     * 同一ユニット内の他の見積先は採用解除（adoption_flg=0）して「項目ごとに選定済みの業者」を確定する。
     *
     * @param  list<int>  $partnerIds  選定された支払取引先（t_payable_partners）の ID 配列
     * @return int 実際に採用を確定した見積先の件数
     */
    public function recordVendorSelections(array $partnerIds): int;

    /**
     * 部長承認。選定された支払取引先（t_payable_partners）を承認済み（fix_status=1）にする。
     *
     * @param  list<int>  $partnerIds  承認する支払取引先（t_payable_partners）の ID 配列
     * @return int 実際に更新した件数
     */
    public function recordManagerApprovals(array $partnerIds): int;

    /**
     * 部長承認の否認（業者選定へ差し戻し）。対象の見積先（担当承認済＝APPLIED）を
     * 未選定（DRAFT）へ戻し、否認理由（deny_comment）を記録する。
     *
     * @param  int  $partnerId  見積先 ID（新スキーマ=t_payable_partners.id）
     * @param  string  $reason  否認理由
     * @return int 実際に差し戻した件数（0=対象外）
     */
    public function rejectManagerApproval(int $partnerId, string $reason): int;
     ...
     *
     * @param  int  $partnerId  見積先 ID（t_payable_partners.id）
     * @return int|null 費用項目 ID（見積先が無ければ null）
     */
    public function itemIdForPartner(int $partnerId): ?int;

    /**
     * 部長取消申請。対象の支払取引先（t_payable_partners）を取消申請中（cancel_flg=1）にする。
     *
     * @param  list<int>  $partnerIds  取消申請する支払取引先（t_payable_partners）の ID 配列
     * @return int 実際に更新した件数
     */
    public function recordCancelRequests(array $partnerIds): int;

    /**
     * 部長取消承認。対象の支払取引先（t_payable_partners）を取消承認済み（cancel_flg=2）にする。
     *
     * @param  list<int>  $partnerIds  取消承認する支払取引先（t_payable_partners）の ID 配列
     * @return int 実際に更新した件数
     */
    public function recordCancelApprovals(array $partnerIds): int;
