...
import { PAYABLE_STATUS_LABEL } from '../model/payable';
import type { PayableMode, PayableProject, PayableRow } from '../model/payable';

const props = defineProps<{
    project: PayableProject;
    mode: PayableMode;
    open: boolean;
    glass?: boolean;
...
const emit = defineEmits<{
    (e: 'toggle'): void;
    (e: 'row-toggle', row: PayableRow): void;
    (e: 'row-action', row: PayableRow): void;
    (e: 'provisional-toggle', row: PayableRow): void;
    (e: 'reject', row: PayableRow): void;
...
// 元データ（billingTarget）は PayablePartnerResource が全モードで返しているため、画面ごとの出し分けは不要。
...
const isActive = (row: PayableRow): boolean => props.selectedKeys.has(payableRowKey(row));
...
const isProvisional = (row: PayableRow): boolean => props.provisionalKeys?.has(payableRowKey(row)) ?? false;
...
interface DisplayRow {
    ...
    /** この項目（budgetItemId）の表示上の先頭行か。項目名セルはここにだけ rowspan で出す。 */
...
// 項目（budgetItemId）内で「もらい（請求先）」を先、「払い（通常）」を後にまとめ、境界に線を引けるようにする。
...
const displayRows = computed<DisplayRow[]>(() => {
    ...
    const byUnit = new Map<number, PayableRow[]>();
    for (const row of props.project.rows) {
        if (!byUnit.has(row.budgetItemId)) {
            byUnit.set(row.budgetItemId, []);
            unitOrder.push(row.budgetItemId);
        }
        byUnit.get(row.budgetItemId)!.push(row);
    }
    const result: DisplayRow[] = [];
    unitOrder.forEach((budgetItemId, unitIndex) => {
        const rows = byUnit.get(budgetItemId) ?? [];
...
const itemHasSelection = (row: PayableRow): boolean =>
    props.project.rows.some((r) => r.budgetItemId === row.budgetItemId && isActive(r));
// メインボタンの状態色：
...
const rowColor = (row: PayableRow): RowColor => {
...
const mainBtnClass = (row: PayableRow): string => {
...
const chatBtnClass = (row: PayableRow): string => {
                ...
                <tbody>
                    ...
                    >
                        ...
                        <td v-if="isUnitFirstRow" :rowspan="unitRowSpan" class="px-3 py-2 font-medium" :class="cardBgClass">
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <div class="flex shrink-0 items-center gap-1.5">
                                    ...
                                    <button
                                        v-if="showChat && row.partnerId != null"
                                        type="button"
                        ...
                        <td v-if="showLastRequestedAt" class="px-3 py-2 text-center">
                            <span
                                v-if="row.partnerId != null && row.lastRequestedAt"
